...
var Menu = class ArcMenu_ArcMenuLayout extends BaseMenuLayout {
    static {
    ...
    _createExtraCategoriesLinks(){
        ...
        for(let i = 0; i < extraCategories.length; i++){
            ...
            if(shouldShow && categoryEnum === Constants.CategoryType.PINNED_APPS && defaultMenuView === Constants.DefaultMenuView.PINNED_APPS)
                continue;
            else if(shouldShow && categoryEnum === Constants.CategoryType.FREQUENT_APPS && defaultMenuView === Constants.DefaultMenuView.FREQUENT_APPS)
                continue;
            else if(shouldShow && categoryEnum === Constants.CategoryType.ALL_PROGRAMS && defaultMenuView === Constants.DefaultMenuView.ALL_PROGRAMS)
                continue;
    ...
    displayFrequentApps(){
        ...
        for (let i = 0; i < mostUsed.length; i++) {
            if (mostUsed[i] && mostUsed[i].get_app_info().should_show()){
                ...
                const item = new MW.ApplicationMenuItem(this, mostUsed[i], Constants.DisplayType.LIST, null, isContainedInCategory);
                appList.push(item);
            }
        }
